...
> **🔹 中文标题：** 《TimeLens：基于检索增强问答的大埃及博物馆在馆文物识别技术》
>
> | 属性 | 内容 |
> |:---:|:---|
> | 📅 发布日期 | 2026-06-11 |
...
>
> **📝 中文摘要：**
> TimeLens是一款基于人工智能的双语移动导览系统，专为大埃及博物馆（GEM）设计。游客将手机对准展品时，系统能实时识别文物并支持英语或阿拉伯语的后续问答交互。该系统针对展厅部署的三个特殊问题：51件馆藏文物的细粒度视觉相似性（包含大量近乎相同的拉美西斯雕像）、精编训练数据与手持设备拍摄条件的差异，以及AI导览生成未经考证历史信息的风险。研究取得了两项工程技术突破：第一，通过数据质量驱动的迭代研究（从基础模型自动标注YOLO-World到空间标签清洗规则，最终形成全手工标注数据集）开发出终端设备文物检测器，证实标签质量是决定性因素——最终的YOLOv8n模型在5.97MB的TensorFlow Lite资源包中实现了所有失败类别的精准识别，可在中端手机实时运行（mAP@0.5=0.995，mAP@0.5:0.95=0.924）。第二，构建了基于108条ChromaDB知识库的双语检索增强生成（RAG）导览系统，经七种候选语言模型评估后选定Gemma 4 E2B（Q4 K M），通过十项定向优化将端到端延迟从30余秒降至约10秒。这两个子系统已集成至具备双语界面、博物馆位置门控和语音合成的生产级Flutter应用程序中。
>
...
> **🔹 中文标题：** YOLO-AMC：一种基于注意力机制的改进YOLO架构在建筑裂缝检测中的应用
>
> | 属性 | 内容 |
> |:---:|:---|
> | 📅 发布日期 | 2026-06-11 |
...
>
> **📝 中文摘要：**
> 裂缝检测在基础设施巡检和结构健康监测中扮演着重要角色。然而，裂缝通常呈现为细微、低对比度的结构，且易受背景噪声影响，这给现有的目标检测模型带来了挑战。本研究提出了一种融合注意力机制的改进型YOLO架构，命名为YOLO-AMC（基于注意力机制的裂缝检测YOLO模型），以提升自动化裂缝检测性能。该模型基于YOLOv11架构，移除了原始C2PSA模块，并在网络颈部的多尺度特征融合层中引入了全局注意力机制、残差卷积块注意力模块和通道注意力等多种注意力机制，以增强跨尺度特征整合能力。实验结果表明，YOLO-AMC在多项评估指标上均优于基线模型YOLOv11n和YOLOv8n。其中，采用全局注意力机制的模型表现最佳，在测试集上达到mAP@0.5 = 0.9917和mAP@0.5:0.95 = 0.9506的检测精度，显著优于YOLOv11（0.9833 / 0.9112）和YOLOv8（0.9707 / 0.8921）。此外，该模型在保持7.6 GFLOPs计算复杂度的前提下，在NVIDIA RTX 4090平台上达到110.95 FPS，在树莓派5边缘设备上可达约5 FPS，实现了精度与部署效率的良好平衡。本研究代码已在GitHub平台开源：https://github.com/CY-Tsai24/YOLO-AMC。
>
...
> **🔹 中文标题：** YOLOv11与YOLOv8在发展中国家恶劣天气条件下混合交通目标检测中的性能分析
>
> | 属性 | 内容 |
> |:---:|:---|
> | 📅 发布日期 | 2026-06-10 |
...
>
> **📝 中文摘要：**
> 在现代车载系统中，严苛条件下的鲁棒性能已成为自动驾驶的关键课题。本研究对最新迭代的YOLO系列——YOLOv11 Nano架构进行全面评估，以广泛采用的YOLOv8 Nano作为基准，在融合印度驾驶数据集(IDD)[1]与伯克利深度驾驶数据集(BDD100K)[2]构建的自定义数据集上进行对比实验。我们针对包含密集混合交通流、降雨及低光照条件的高熵场景，系统分析了检测精度、推理速度与计算效率之间的权衡关系。研究发现：YOLOv11n在50%交并比阈值下达到46.6%的平均精度均值(mAP@50)，相较基准模型精确度提升3.2个百分点，有效降低了复杂场景中的误报率。该模型在能效方面表现更优，浮点运算量减少22%（6.3G对比8.1G），同时在Tesla T4 GPU上仍保持70.9 FPS的实时推理速度，为安全关键型边缘部署提供了最优平衡方案。
>
...
> **🔹 中文标题：** 基于骨髓涂片深度学习分析的急性髓系白血病患者层面诊断
>
> | 属性 | 内容 |
> |:---:|:---|
> | 📅 发布日期 | 2026-06-09 |
...
>
> **📝 中文摘要：**
> 骨髓涂片检查在急性髓系白血病评估中仍具重要价值，但人工单细胞判读耗时费力，且患者级诊断需要整合大量细胞观察结果。本研究提出一种从骨髓涂片图像辅助诊断AML的"细胞到患者"深度学习流程。研究纳入来自六个匿名中心的258例患者，其中中心1-3的169例构成主队列，中心4-6的89例构成外部验证队列。采用包含16个类别的细胞标注词库描述全局细胞组成，涵盖粒细胞系、单核细胞系、红系、淋巴系、嗜酸性粒细胞等细胞类型。该模型不针对严格的AML原始细胞或白血病原始细胞判别，而是聚焦于专家定义的复合类别——原幼细胞样细胞，依据全项目形态学标准涵盖N、N1、M、M1、R、R1、J、J1等亚型。固定YOLO分割模块检测细胞后，通过轮廓IoU匹配预测轮廓与专家多边形标注，生成标准化单细胞裁剪图。采用两阶段GT-to-YOLO与YOLO-to-YOLO策略训练EfficientNet-B0分类器，结合类别不平衡校正、中心-边界正则化及形态学辅助监督。细胞级预测结果经聚合生成患者级CBLC比例，为AML导向的诊断提供支持。该流程在内部验证中表现稳定，外部泛化能力良好，在中心4、5、6的集成加权F1分数分别为0.9076、0.8696和0.9124。
>
...
>
> **📝 中文摘要：**
> 被动声学监测技术实现了对野生动物的大规模观察，但大多数生物声学分类器仅能预测物种在时间窗口内的存在性，无法精确定位发声事件的时间或频率位置，从而限制了后续分析。本研究将鸟类鸣叫检测建模为频谱图上的目标检测任务，并使用YOLO11模型在新加坡密集热带声景中进行鸟类叫声定位。我们进一步开发了开源浏览器标注工具，并提出最小交并比（IoMin）评估指标，该指标相比标准IoU能更好地处理模糊声学边界，更适用于当前研究问题。最优YOLO模型在新加坡同分布声景中的表现近乎超越基线模型一倍（81.8% vs. 42.1% IoMin@50 F1分数），同时在夏威夷未见过的异分布录音中仍保持性能优势（58.6% vs. 48.6%）。这些结果表明，目标检测框架在复杂声景中实现动物发声事件时频定位具有广阔前景。
>
